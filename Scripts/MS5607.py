...
class MS5607:
    ...
    def getCompTemperature(self):
        #SB - 2016-06-06
        dT = self.getDigitalTemperature() - self.coefficients[4] * math.pow(2, 8)
        temp =  (2000 + dT * self.coefficients[5] / math.pow(2, 23))

        if temp < 2000 :
            T2 = math.pow(dT,2) / math.pow(2,31)
            OFF2 = 61*math.pow((temp - 2000),2)/math.pow(2,4)
            SENS2 = 2*math.pow((temp-2000),2)
        else :
            T2 = 0
            OFF2 = 0
            SENS2 = 0
        ...
        if temp < -1500:
            OFF2 = OFF2 + 15 * math.pow((temp + 1500), 2)
            SENS2 = SENS2 + 8 * math.pow((temp + 1500), 2)
