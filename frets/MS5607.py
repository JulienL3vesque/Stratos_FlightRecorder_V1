...
class MS5607:
    ...
    def read16U(self, register1, register2):
      if self.isValid:
          bytes = self.bus.read_i2c_block_data(self.DEVICE_ADDRESS, register1, 2)
    ...
    def read24U(self, register):
      if self.isValid:
          bytes = self.bus.read_i2c_block_data(self.DEVICE_ADDRESS, register, 3)
    ...
    def resetSensor(self):
      try:
        self.bus.write_byte(self.DEVICE_ADDRESS, self._CMD_RESET)
    ...
    def readAdc(self, cmd):
      if self.isValid:
          # set conversion mode
          self.bus.write_byte(self.DEVICE_ADDRESS, self._CMD_ADC_CONV + cmd)
